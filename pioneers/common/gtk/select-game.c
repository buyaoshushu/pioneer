...
GType select_game_get_type(void)
{
	...
	if (!sg_type) {
		...
		sg_type = g_type_register_static(
				GTK_TYPE_TABLE, "SelectGame",
				&sg_info, 0);
	}
	return sg_type;
}
...
static void select_game_class_init(SelectGameClass *klass)
{
	select_game_signals[ACTIVATE] = g_signal_new(
			"activate",
			G_TYPE_FROM_CLASS(klass),
			G_SIGNAL_RUN_FIRST | G_SIGNAL_ACTION,
			G_STRUCT_OFFSET(SelectGameClass, activate),
			NULL,
			NULL,
...
void select_game_add(SelectGame *sg, const gchar *game_title)
{
	...
	if (!strcmp(game_title, sg->default_game)) {
		...
		gtk_menu_shell_prepend(GTK_MENU_SHELL(sg->menu), separator);
