...
static void admin_write(gchar *fmt, ...)
{
	...
	va_start(ap, fmt);
	g_vsnprintf(&buff[ADMIN_PREFIX_LEN], ADMIN_BUFSIZE - ADMIN_PREFIX_LEN, 
		fmt, ap);
	va_end(ap);
...
{
	gint server_port = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(widget));
	
	admin_write( "set-port %d\n", server_port );
}
