...
struct chromosome_t thisChromosome = (struct chromosome_t) {
    {
        {1.3,1,1,1,1.3, 1,0.5},	/*weight of every resource and Development Card when I have 0 Victory Points*/
        {1.2,1,1,1,1, 1.2,0.5},	/*when I have 1 */
        {1,1,1,1,1, 1,0.7},	/*2 */
        {1,1.2,1.2,1.2,1, 2,0.8},	/*3*/
        {1,1.3,1.3,1.3,1, 2,0.9},	/*4*/
        {1,1,1,1,1, 2,1},	/*5*/
        {1,1,1,1,1, 2,1},	/*6*/
        {1,1,1,1,1, 2,1.1},	/*7*/
        {1,1,1,1,1, 2,1.2},	/*8*/
    ...
    0.05,4,0.60	/*depreciation, turn and probability*/
};/*default chromosome*/
...
static Edge *best_road_to_road_spot(Node * n, float *score,
                                    const struct chromosome_t *myChromosome, const struct gameState_t *myGameState);

static Edge *best_road_to_road(const struct chromosome_t *myChromosome, const struct gameState_t *myGameState,float *destinationScore);

static Edge *best_road_spot(const struct chromosome_t *myChromosome, const struct gameState_t *myGameState, float *destinationScore);

static Node *best_city_spot(const struct chromosome_t *myChromosome, const struct gameState_t *myGameState);
...
void updateTradingMatrix(const struct chromosome_t *myChromosome, float profit,
...
void outputStrategy(strategy_t myStrategy,const struct simulationsData_t mySimulation,const struct gameState_t myGameState);

float best_maritime_trade(const struct tradingMatrixes_t thisTradingMatrixes, int *amount, Resource *trade_away, Resource *want_resource);
...
static void genetic_for_each_node(iterate_node_func_t * func, void *rock,node_seen_set_t *nodesSeen)
{
    Map *map;
    int i, j, k;
                           ...
                           gint need[NO_RESOURCE])
{
    ...
    case BUY_SETTLEMENT:
        return (stock_num_settlements() > 0 &&
                best_settlement_spot(FALSE,myChromosome, myGameState) != NULL);
    case BUY_ROAD:
        /* don't sprawl :) */
        return (stock_num_roads() > 0 &&
                places_can_build_settlement() <= 2 &&
                (best_road_spot(myChromosome, myGameState,&destinationScore) != NULL ||
                 best_road_to_road(myChromosome, myGameState,&destinationScore) != NULL));
    case BUY_DEVEL_CARD:
    ...
    if ((n) && (n->owner == my_player_num())) {
        ...
        for (l = 0; l < 3; l++) {
            ...
            if (h && h->terrain < DESERT_TERRAIN) {
                ...
                myGameState->resourcesSupply[i][j]+= supply;
            }
...
static void reevaluate_gameState_supply_matrix_and_resources(struct gameState_t * myGameState)
/*Updates the information in struct gameState_t pointed by myGameState*/
{
    ...
    genetic_for_each_node(&genetic_reevaluate_iterator, (void *) myGameState,&nodesSeen);
...
static float genetic_score_hex(Hex *hex, const struct chromosome_t *myChromosome, const struct gameState_t *myGameState)
{
    Resource resrc;
    ...
    int port_bonus=0;/*bonus for being a port*/
    resrc=terrain_to_resource(hex->terrain);
    if (resrc < NO_RESOURCE) {
        map_maritime_info(callbacks.get_map(),&info,my_player_num());
        /*I want to decrease the devaluation of the resource of an hex if a I have a generic port or a specific port to export that resource*/
        if (info.any_resource) port=3;/*I have a generic port, depreciation will be less*/
        else if (info.specific_resource[resrc]) port=2;/*I have a port to export this resource*/
        /*Also I want to give a special value to ports*/
        if ((!info.any_resource)&&(hex->resource==ANY_RESOURCE)) port_bonus=1;
        /*
        Here it should check also for specific port, and its value should be increased proportionally
        to the Average Resource Supplay of that resource I already have
          */

        value=resourcesIncrementValue(increment,resrc,victoryPoints,myChromosome,myGameState,port);
...
static float genetic_score_node(const Node * node, gboolean city,const struct chromosome_t *myChromosome, const struct gameState_t *myGameState)
{
    int i;
    float score = 0;
    ...
    for (i = 0; i < 3; i++) {
        score += genetic_score_hex(node->hexes[i], myChromosome, myGameState);
    }
    ...
    for (i = 0; i < map->x_size; i++) {
        for (j = 0; j < map->y_size; j++) {
            for (k = 0; k < 6; k++) {
                ...
                score = genetic_score_node(n, FALSE, myChromosome, myGameState);
                if (score > bestscore) {
                    best = n;
                    bestscore = score;
                }
        ...
        for (j = 0; j < map->y_size; j++) {
            for (k = 0; k < 6; k++) {
                ...
                if ((n->owner == my_player_num())
                    ...
                    float score =
                        genetic_score_node(n, TRUE, myChromosome, myGameState);
                          ...
                          const struct chromosome_t *myChromosome, const struct gameState_t *myGameState)
{
    ...
    for (i = 0; i < 3; i++) {
        ...
        if (e->owner == my_player_num()) {
            ...
            if (!nodeset_isset(set, othernode))
                cur_e =
                    traverse_out(othernode, set,
                                 &cur_score, myChromosome, myGameState);
            ...
            cur_score = genetic_score_node(othernode, FALSE, myChromosome, myGameState);
            ...
            if (can_road_be_built(e, my_player_num())) {
                ...
                for (j = 0; j < 3; j++) {
                    ...
                    if (can_road_be_built
                            (e2, my_player_num())) {
                        float nscore =
                            genetic_score_node(other_node(e2,othernode),
                                               FALSE,
            ...
            for (k = 0; k < 6; k++) {
                ...
                if ((n != NULL)
                    ...
                    e = traverse_out(n, &nodeseen,
                                     &score, myChromosome, myGameState);
 ...
 */
static int resource_desire(gint assets[NO_RESOURCE],const struct chromosome_t *myChromosome, const struct gameState_t *myGameState)
{
    ...
    for (bt = 0; bt < BUY_LAST; bt++) {
        if (should_buy(assets, bt, myChromosome, myGameState, need))
            continue;
        res = which_one(need);
        if (res == NO_RESOURCE )
            continue;
...
static void genetic_setup_road(void)
{
    ...
    e = best_road_to_road_spot(node, &tmp, &thisChromosome, &myGameState);
                         ...
                         struct gameState_t myGameState)
{
    ...
    for (give = 0; give <= 4; give++) {
        if ((myGameState.resourcesAlreadyHave[give] >= 2)&&(info.specific_resource[give])) { /*I have a specific port to export give*/
            for (take = 0; take <= 4; take++) {
                if (give != take) {	/*There is no point in trading the same resource */
                    myGameState.resourcesAlreadyHave[give]-=2;
                    myGameState.resourcesAlreadyHave[take]++;
                    profitAfterTrade = bestStrategy(turn, prob,&thisSimulation,thisStrategy,myGameState, 0);
                    if (profitAfterTrade > profit) {
                        tradeThisForThat->specificResource[give][take] = profitAfterTrade;
                    }
                    myGameState.resourcesAlreadyHave[give] +=2;
                    myGameState.resourcesAlreadyHave[take]--;
                }
        ...
        /*It will check this only if it is not possible to do 2:1 trade*/
            ...
            for (take = 0; take <= 4; take++) {
                if (give != take) {	/*There is no point in trading the same resource */
                    myGameState.resourcesAlreadyHave[give]-=3;
                    myGameState.resourcesAlreadyHave[take]++;
                    profitAfterTrade = bestStrategy(turn, prob,&thisSimulation,thisStrategy,myGameState, 0);
                    if (profitAfterTrade > profit) {
                        tradeThisForThat->genericResource[give][take] = profitAfterTrade;
                    }
                    myGameState.resourcesAlreadyHave[give] +=3;
                    myGameState.resourcesAlreadyHave[take]--;
                }
        ...
        /*It will check this only if it is neither possible to do 2:1 nor 3:1 trade*/
            ...
            for (take = 0; take <= 4; take++) {
                if (give != take) {	/*There is no point in trading the same resource */
                    myGameState.resourcesAlreadyHave[give]-=4;
                    myGameState.resourcesAlreadyHave[take]++;
                    profitAfterTrade = bestStrategy(turn, prob,&thisSimulation,thisStrategy,myGameState, 0);
                    if (profitAfterTrade > profit) {
                        tradeThisForThat->bankTrade[give][take] = profitAfterTrade;
                    }
                    myGameState.resourcesAlreadyHave[give] +=4;
                    myGameState.resourcesAlreadyHave[take]--;
                }
        ...
        /*At this point the highest value of the these 3 matrixes represents the most beneficial trade of all*/
...
/*Inspects thisTradingMatrixes and outputs the most favorable maritime trade*/
{
    ...
    for (give=0; give<5; give++) {
        for (take=0; take<5; take++) {
            if (take!=give) {
                if (thisTradingMatrixes.bankTrade[give][take]>best) {
                    best=thisTradingMatrixes.bankTrade[give][take];
                    portType=4;
                    bestgive=give;
                    besttake=take;
                }
                if (thisTradingMatrixes.specificResource[give][take]>best) {
                    best=thisTradingMatrixes.specificResource[give][take];
                    portType=2;
                    bestgive=give;
                    besttake=take;
                }
                if (thisTradingMatrixes.genericResource[give][take]>best) {
                    best=thisTradingMatrixes.genericResource[give][take];
                    portType=3;
                    bestgive=give;
                    besttake=take;
                }
...
void outputGameState(const struct gameState_t myGameState) {
    ...
    printf("\t\tActionValues:\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\n\n",myGameState.actionValue[SET],myGameState.actionValue[CIT],myGameState.actionValue[DEV],myGameState.actionValue[RSET],myGameState.actionValue[RRSET]);
    printf("\t\t\t\tBr\tGr\tOr\tWo\tLu\n");
    printf("\t\tResources:");
    for (i = 0; i < NO_RESOURCE; ++i) printf("\t%d",myGameState.resourcesAlreadyHave[i]);
    printf("\n");
...
void outputStrategy(strategy_t myStrategy,const struct simulationsData_t mySimulation,const struct gameState_t myGameState) {
    ...
    time_b = mySimulation.timeCombinedAction[myStrategy[0]][myStrategy[1]];
    ...
    else if (((myStrategy[0]==RSET)||(myStrategy[0]==RRSET)||(myStrategy[0]==CIT))&&(checkRoadNow(myStrategy[0],myGameState))) {
        printf("\t\tI cannot do ");
        printAction(myStrategy[0]);
...
static void genetic_turn(void)
{
    ...
    if ((city_node != NULL)&&(stock_num_cities())) {
        thisGameState.actionValue[CIT]=genetic_score_node(city_node,TRUE,&thisChromosome, &thisGameState)
	*(thisChromosome.resourcesValueMatrix[victoryPoints][6]);/*The relative value of city at this point in the game*/
    } else thisGameState.actionValue[CIT]=0;


    /*Best Settlement*/
    sett_node = best_settlement_spot(FALSE,&thisChromosome, &thisGameState);
    if ((sett_node != NULL)&&stock_num_settlements()) {
        thisGameState.actionValue[SET]=genetic_score_node(sett_node,FALSE,&thisChromosome, &thisGameState);
    } else thisGameState.actionValue[SET]=0;

    /*Best Road to Settlement, that is, best destination to build a Settlement after building a Road*/
    road_edge = best_road_spot(&thisChromosome, &thisGameState, &destinationRoadScore);
    if ((road_edge != NULL)&&stock_num_roads()&&stock_num_settlements()) {
    ...
    long_road_edge = best_road_to_road(&thisChromosome, &thisGameState, &destinationLongRoadScore);
    if ((long_road_edge != NULL)&&(stock_num_roads()>=2)&&stock_num_settlements()) {
        thisGameState.actionValue[RRSET]=destinationLongRoadScore;
    }

    /*value of buying Development Card is fixed in the chromosome*/
    thisGameState.actionValue[DEV]=thisChromosome.resourcesValueMatrix[victoryPoints][5];



    outputGameState(thisGameState);

    strategyProfit=bestStrategy(turn,probability,&thisSimulation,thisStrategy,thisGameState,0);
    printf("\t\t\t\tSET\tCIT\tDEV\tRSET\tRRSET\tS+SET\tS+CIT\tS+DEV\tS+RSET\tS+RRSET\tC+CIT\tC+DEV\tC+RSET\tC+RRSET\tD+DEV\tD+RSET\tD+RRSET\tR+RSET\tR+RRSET\tRR+RRSET\n");
    printf("\t\tTime to Action:\t");
    for (i=0; i<NUM_ACTIONS; i++) printf("%d\t",thisSimulation.turnsToAction[i]);
    printf("\n");
    ...
    updateTradingMatrix(&thisChromosome,strategyProfit,&thisTradingMatrixes,thisGameState);
    if (best_maritime_trade(thisTradingMatrixes,&amount,&trade_away,&want_resource)&&can_trade_maritime()) {
        printf("I will trade %d of %d for 1 of %d\n",amount,trade_away,want_resource);
    ...
    if (time_a==0) {/*I can do what I want NOW*/
        ...
        * so we should check that there is in fact possible to do what I want to do before trying to do it*/
        for (i=0; i<5; i++) printf(" %d ",resource_asset(i));
    ...
    else if (((thisStrategy[0]==RSET)||(thisStrategy[0]==RRSET))&&(checkRoadNow(thisStrategy[0],thisGameState))&&stock_num_roads()) {
        /*I want to build RSET or RRSET and, although I cannot do it completely now, I can build road*/
        if (thisStrategy[0]==RSET) {
    ...
    else if ((thisStrategy[0]==CIT)&&(checkRoadNow(thisStrategy[0],thisGameState))&&stock_num_roads()) {
        /*I want to do city, but in the meantime I can build a road (that has no effect on city) and there is a place to do it*/
...
static void genetic_free_road(void)
{
    ...
    */
    ...
    e = best_road_spot(&thisChromosome, &myGameState,&destinationScore);

    if (e == NULL) {
        e = best_road_to_road(&thisChromosome, &myGameState, &destinationScore);
    }
...
static gint determine_monopoly_resource()
{
    ...
    most_desired = resource_desire(assets, &thisChromosome, &myGameState);
...
static void reevaluate_gameState_actionValues(const struct chromosome_t *myChromosome, struct gameState_t *myGameState) {
    ...
    if ((city_node != NULL)&&(stock_num_cities())) {
        myGameState->actionValue[CIT]=genetic_score_node(city_node,TRUE,myChromosome, myGameState);
    } else myGameState->actionValue[CIT]=0;


    /*Best Settlement*/
    sett_node = best_settlement_spot(FALSE,myChromosome, myGameState);
    if ((sett_node != NULL)&&stock_num_settlements()) {
        myGameState->actionValue[SET]=genetic_score_node(sett_node,FALSE,myChromosome, myGameState);
    } else myGameState->actionValue[SET]=0;

    /*Best Road to Settlement, that is, best destination to build a Settlement after building a Road*/
    road_edge = best_road_spot(myChromosome, myGameState, &destinationRoadScore);
    if ((road_edge != NULL)&&stock_num_roads()&&stock_num_settlements()) {
        myGameState->actionValue[RSET]=destinationRoadScore;
    } else myGameState->actionValue[RSET]=0;

    /*Best Long Road to Settlement, this is, best destination to build a Settlement after building two Roads*/
    long_road_edge = best_road_to_road(myChromosome, myGameState, &destinationLongRoadScore);
    if ((long_road_edge != NULL)&&(stock_num_roads()>=2)&&stock_num_settlements()) {
        myGameState->actionValue[RRSET]=destinationLongRoadScore;
    } else myGameState->actionValue[RRSET]=0;

    /*value of buying Development Card is fixed in the chromosome*/
    myGameState->actionValue[DEV]=myChromosome->resourcesValueMatrix[victoryPoints][5];
    return;

}

static int update_todiscard_resources(int totalDiscards, const struct chromosome_t *myChromosome, struct gameState_t *myGameState, gint todiscard[NO_RESOURCE])
/*given totalDiscards resources I should discard it sets the array todiscard to that resources*/
{
    ...
    while (discards<totalDiscards) {
        ...
        actualProfit=bestStrategy(turn, prob,&thisSimulation,thisStrategy,*myGameState, 0);
        for (give = 0; give <= 4; give++) {/*which resource is the best to get rid of it?*/
            if (myGameState->resourcesAlreadyHave[give] >= 1) { /*I have something to discard*/
                ...
                profitLossAfterDiscard = actualProfit-bestStrategy(turn, prob,&thisSimulation,thisStrategy,*myGameState, 0);
                if (profitLossAfterDiscard < minimumProfitLoss) {/*this discard is the best so far*/
                    minimumProfitLoss=profitLossAfterDiscard;
                    giveaway=give;
                }
                myGameState->resourcesAlreadyHave[give]++;
            }
        ...
        discards++;
        printf("The resource number %d of the total %d I have to discard chosen is resource %d (now I have %d left)\n",
               discards,totalDiscards,giveaway,myGameState->resourcesAlreadyHave[giveaway]);
    }

    if ((giveaway==-1)||(discards!=totalDiscards)) printf("giveaway=-1!!! or wrong number of discards\n");
    return (discards);
...
static void genetic_discard(int num)
{
    ...
    update_todiscard_resources(num, &thisChromosome, &myGameState,todiscard);
                         ...
                         gboolean free_offer)
{
    ...
    for (n = 1; n <= 3; ++n) {
        ...
        if (!should_buy(assets, BUY_CITY, &thisChromosome, &myGameState,need)) {
            if ((res = which_resource(need)) == take
                    && need[res] == n)
                break;
        }
        if (!should_buy(assets, BUY_SETTLEMENT,&thisChromosome, &myGameState, need)) {
            if ((res = which_resource(need)) == take
                    && need[res] == n)
                break;
        }
        if (!should_buy(assets, BUY_ROAD, &thisChromosome, &myGameState,need)) {
            if ((res = which_resource(need)) == take
                    && need[res] == n)
                break;
        }
        if (!should_buy(assets, BUY_DEVEL_CARD, &thisChromosome, &myGameState,need)) {
            if ((res = which_resource(need)) == take
                    && need[res] == n)
                break;
        }
...
static void genetic_discard_start(void)
{
...
static void genetic_error(const gchar * message)
{
    gchar *buffer;
...
static void genetic_player_turn(gint player)
{
    if (player == my_player_num())
        randchat(chat_turn_start, 70);
}

static void genetic_robber_moved(G_GNUC_UNUSED Hex * old, Hex * new)
{
    guint idx;
...
static void genetic_player_robbed(G_GNUC_UNUSED gint robber_num,
                                  gint victim_num,
...
static void genetic_get_rolled_resources(gint player_num,
...
static void genetic_played_develop(gint player_num,
...
static void genetic_new_statistics(gint player_num, StatisticType type,
                                   gint num)
{
    if (num != 1)
        return;
...
void genetic_init(void)
{
    ...
    callbacks.error = &genetic_error;

    /* chatting */

    callbacks.player_turn = &genetic_player_turn;
    callbacks.robber_moved = &genetic_robber_moved;
    callbacks.discard = &genetic_discard_start;
    callbacks.player_robbed = &genetic_player_robbed;
    callbacks.get_rolled_resources = &genetic_get_rolled_resources;
    callbacks.played_develop = &genetic_played_develop;
    callbacks.new_statistics = &genetic_new_statistics;
