 ...
 */
void config_init( gchar *path_prefix );
 ...
 */
gchar *config_get_string( gchar* path, gboolean* default_used );

/* get an integer.  If a default is sent as part of the path, and the
 * default is returned, set *default_used to 1.
 */
gint config_get_int( gchar* path, gboolean* default_used );
...
void config_set_string( gchar* path, const gchar* value );

/* set an int; make sure the configuration set is sync'd afterwards. */
void config_set_int( gchar* path, gint value );
