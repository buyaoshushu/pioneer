...
# Source code
wget -o /dev/null -O - "http://sourceforge.net/project/showfiles.php?group_id=5095&package_id=5139" | gawk '$0 ~ /pioneers-.*.tar.gz/ { a = match($0, ".*pioneers-(.*).tar.gz", b); if (a==1) { split(b[1], c, "."); printf "%06d %s\n", c[1]*10000+c[2]*100+c[3], b[1] } }' | sort -u -r | gawk '{ print "Source," $2 }' | head -1
...
# Microsoft Windows
wget -o /dev/null -O - "http://sourceforge.net/project/showfiles.php?group_id=5095&package_id=180259" | gawk '$0 ~ "-setup.exe" { a = match($0, ".*Pioneers-(.*)-setup.exe", b); if (a==1) { split(b[1], c, "."); printf "%06d %s\n", c[1]*10000+c[2]*100+c[3], b[1] } }' | sort -u -r | gawk '{ print "Microsoft Windows," $2 }' | head -1
...
wget -o /dev/null -O - "http://sourceforge.net/project/showfiles.php?group_id=5095&package_id=181682" | gawk '$0 ~ /pioneers-[0-9].*.rpm/ { a = match($0, ".*pioneers-(.*).rpm", b); if (a==1) { split(b[1], c, "."); printf "%06d %s\n", c[1]*10000+c[2]*100+c[3], b[1] } }' | sort -u -r | gawk '{ print "RedHat/openSUSE RPM," $2 }' | head -1
