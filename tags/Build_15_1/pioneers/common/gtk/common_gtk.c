...
/* Embedded copy warning...
 * The following is an unmodified embedded copy of Gtk3/Gdk3 code
 *
 * It was taken from the Debian source code for Gtk3: 3.0-3.4.2
 * From the file gdk/gdkpixbuf-drawable.c
 * That file is licensed under LGPL version 2 or later
 * That file has: Copyright (C) 1999 Michael Zucchi
 * And mentions the following authors:
...
static cairo_surface_t *gdk_cairo_surface_coerce_to_image(cairo_surface_t *
							  surface,
							  cairo_content_t
							  content,
							  int src_x,
							  ...
							  int height)
{
	...
	copy =
	    cairo_image_surface_create(gdk_cairo_format_for_content
...
convert_alpha(guchar * dest_data,
	      int dest_stride,
	      guchar * src_data,
	      int src_stride, int src_x, int src_y, int width, int height)
{
	int x, y;
	...
	for (y = 0; y < height; y++) {
		...
		for (x = 0; x < width; x++) {
			...
			if (alpha == 0) {
				...
				dest_data[x * 4 + 0] =
				    (((src[x] & 0xff0000) >> 16) * 255 +
				     alpha / 2) / alpha;
				dest_data[x * 4 + 1] =
				    (((src[x] & 0x00ff00) >> 8) * 255 +
				     alpha / 2) / alpha;
				dest_data[x * 4 + 2] =
				    (((src[x] & 0x0000ff) >> 0) * 255 +
				     alpha / 2) / alpha;
			}
...
GdkPixbuf *gdk_pixbuf_get_from_surface(cairo_surface_t * surface,
				       gint src_x,
				       gint src_y, gint width, gint height)
{
