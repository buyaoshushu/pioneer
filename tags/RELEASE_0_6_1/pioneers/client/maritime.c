...
static void maritime_cb(void *widget, gpointer user_data)
{
	...
	/* Switch everything off and wait for response
	 */
	gtk_widget_set_sensitive(maritime_btn, FALSE);
	for (idx = 0; idx < numElem(maritime_ratio); idx++)
		gtk_widget_set_sensitive(maritime_ratio[idx], FALSE);
	for (idx = 0; idx < numElem(maritime_supply); idx++)
		gtk_widget_set_sensitive(maritime_supply[idx], FALSE);
