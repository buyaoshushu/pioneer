 ...
 */
static Node *best_settlement_spot(int connected, resource_values_t *resval)
{
    int i, j, k;
    Node *best = NULL;
    float bestscore = -1.0;
	...
	for (j = 0; j < map->y_size; j++) {
		...
		if ((connected) && (!road_connects(n))) continue;
...
static Node *best_city_spot(resource_values_t *resval)
{
	...
	for (j = 0; j < map->y_size; j++) {
		...
		if ((n != NULL) && (n->owner == my_player_num ()) && 
 ...
 *
 */
static Edge *traverse_out(Node *n, node_seen_set_t *set, float *score, resource_values_t *resval)
{
    float bscore = 0.0;
    Edge *best = NULL;
    int i;
	...
	if (e->owner == my_player_num ()) {
		...
		cur_e = traverse_out(othernode, set, &cur_score, resval);
...
static Edge *best_road_to_road(resource_values_t *resval)
{
	...
	for (j = 0; j < map->y_size; j++) {
		...
		if ((n) && (n->owner == my_player_num ())) {
		    e = best_road_to_road_spot(n, &score, resval);
		    if (score > bestscore) {
			best = e;
			bestscore = score;
		    }		    
...
static Edge *best_road_spot(resource_values_t *resval)
{
	...
	for (j = 0; j < map->y_size; j++) {
		...
		if ((n!=NULL) && (n->owner == my_player_num ())) {
		    ...
		    e = traverse_out(n, &nodeseen, &score, resval);
...
static void findit_iterator(Node *n, void *rock)
{
    ...
    if (!n) return;
    if (n->owner != my_player_num ()) return;
		      ...
		      gint we_supply[NO_RESOURCE])
{
    ...
    gint give_res[NO_RESOURCE], take_res[NO_RESOURCE], my_assets[NO_RESOURCE];
    int i;
	...
	for (take = 0; take < NO_RESOURCE; take++) {
	    if (!we_receive[take])
		continue;
	    if ((ntake = trade_desired(my_assets, give, take)) > 0)
		goto doquote;
	}
    }
...
void greedy_init (UNUSED (int argc), UNUSED (char **argv) )
{
	...
	if (map_traverse (map, &greedy_check_gold, NULL) == TRUE) {
		cb_chat (N_("Sorry, I do not know how to play with gold.\n") );
		exit (1);
	}
