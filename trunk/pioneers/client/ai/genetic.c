...
struct chromosome_t thisChromosome = (struct chromosome_t) {
    {
        {1.371242,1.984368,1.336144,1.111876,1.206090,0.052862,1.506242,1.684672},
...
void outputGameState(const struct gameState_t myGameState)
{
    ...
    printf("\nStock of:\tRoads %d\tSettlements %d\tCities %d\tDev. Cards %d\n",stock_num_roads(),stock_num_settlements(),stock_num_cities(),stock_num_develop());
}
...
static void genetic_turn(void)
{
    ...
    else printf("Acording to trading matrixes there is no favorable trade possible\n");
    ...
    if (time_a == 0) {	/*I can do what I want NOW */
        ...
        case RSET: {
            if ((road_edge != NULL)&& (stock_num_roads())) {
                printf("Building Road as part of RSET...\n");
        ...
        case RRSET: {
            if ((long_road_edge != NULL)&& (stock_num_roads())) {
                printf("Building Road as part of RRSET...\n");
        ...
        case DEV: {
            if (can_buy_develop()) {
                printf("Buying Development Card...\n");
                cb_buy_develop();
                return;
            }
            break;
        ...
        case DEV:
            ...
            case RSET:
                if (destinationRoadScore) {
                    printf("Building Road of RSET in the meantime...\n");
                    cb_build_road(road_edge);
                    return;
                }
                break;
            case RRSET:
                if (destinationLongRoadScore) {
                    printf("Building Road of RRSET in the meantime...\n");
                    cb_build_road(long_road_edge);
                    return;
                }
                break;
...
static void outputChromosome(void) {
    ...
    for (i=0; i<=9; i++) {
        for (j=0; j<=7; j++) {
            printf("%.3f\t",thisChromosome.resourcesValueMatrix[i][j]);
        }
        printf("\n");
    }
    printf("%.3f\t%.3f\t%.3f\n",thisChromosome.depreciation_constant, thisChromosome.turn, thisChromosome.probability);
}

static void genetic_game_over(gint player_num, G_GNUC_UNUSED gint points)
{
    if (player_num == my_player_num()) {
        printf("FINAL RESULT GENETIC: I won!  (%s) with %2d points using ", my_player_name(),player_get_score(my_player_num()));
        ...
        printf("FINAL RESULT GENETIC: I lost! (%s) with %2d points using ", my_player_name(),player_get_score(my_player_num()));
        if (default_chromosome_used) {
            printf("DEFAULT\n");
        }
        else printf("%s\n",chromosomeFile);
...
static void genetic_init_game(void) {
    ...
    if ((chromFilePointer=fopen(chromosomeFile,"r"))==NULL) {
        printf("Opening of chromosome file %s failed, default used\n",chromosomeFile);
        return;
    }
    printf("Reading chromosome file: %s...\n",chromosomeFile);
    for (i=0; i<=9; i++) {
        if (fgets(line,80,chromFilePointer)==NULL) {
            printf("Some values in the chromosome are missing! Using default then...\n");
            return;
        }
        if (sscanf(line,"%f %f %f %f %f %f %f %f", &tempChromosome.resourcesValueMatrix[i][0],&tempChromosome.resourcesValueMatrix[i][1],&tempChromosome.resourcesValueMatrix[i][2],
                   &tempChromosome.resourcesValueMatrix[i][3],&tempChromosome.resourcesValueMatrix[i][4],&tempChromosome.resourcesValueMatrix[i][5],&tempChromosome.resourcesValueMatrix[i][6],
    ...
    if (sscanf(line,"%f %f %f",&(tempChromosome.depreciation_constant), &(tempChromosome.turn), &(tempChromosome.probability))!=3) {
        printf("Some values in the chromosome are missing! Using default then...\n");
        return;
    }
    printf("%s",line);
    ...
    for (i=0; i<=9; i++) {
        for (j=0; j<=7; j++) {
            thisChromosome.resourcesValueMatrix[i][j]=tempChromosome.resourcesValueMatrix[i][j];
        }
    }
    thisChromosome.depreciation_constant=tempChromosome.depreciation_constant;
